...
class MyCanvasView(context: Context) : View(context) {
    ...
    private fun touchMove() {
        ...
        if (dx >= touchTolerance || dy >= touchTolerance) {
            ...
            Log.d("controllerX", "${(motionTouchEventX + currentX) / 2}")
            Log.d("ControllerY", "${(motionTouchEventY + currentY) / 2}")
            Log.d("endPointX", "$motionTouchEventX")
            Log.d("endPointY", "$motionTouchEventY")
            currentX = motionTouchEventX
            currentY = motionTouchEventY
            // Draw the path in the extra bitmap to cache it.
            // As the user draws on the screen, your app constructs the path and saves it in the
            // bitmap extraBitmap. The onDraw() method displays the extra bitmap in the view's canvas
